...
class File extends \fxyin\Facade
{
     ...
     * @return mixed
     */
    public static function getList($var, $ext = null, $limit = -1)
    {
        // 初始化变量
        $cace['list'] = [];
        ...
        foreach ($files as $file) {
            ...
            if (is_file($cace['info']) && (in_array(pathinfo($file, PATHINFO_EXTENSION), $ext_list) || is_null($ext))) {
                ...
                $cace['child'] = static::getList($cace['info'], $ext, $limit);
     ...
     * @return mixed
     */
    public static function deleteDirectory($var)
    {
        // 初始化变量
        $cace = [];
        $var = realpath($var);
        ...
        foreach ($files as $file) {
            ...
            if (is_file($cace['info'])) {
                ...
                static::deleteDirectory($cace['info']);
            }
        }
        @rmdir($var);
        return true;
     ...
     * @return mixed
     */
    public static function getMediaInfo($var, $type)
    {
        switch ($type) {
            case 'video':
                // 媒体-视频
