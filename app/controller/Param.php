...
class Param extends \fxyin\Facade
{
     ...
     * @return array
     */
    public static function define($param, $mode = null)
    {
        // 初始化变量
        $echo = [];
        switch ($mode) {
     ...
     * @return array
     */
    public static function pick($base, $data)
    {
        // 初始化变量
        $echo = [];
        if (!is_array($base)) {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_array($value)) {
                if (array_key_exists($key, $base)) {
                    $echo[$key] = static::pick($base[$key], $value);
                } else {
                    $echo[$key] = static::pick([], $value);
                }
            } else {
    ...
    public static function merge(...$args)
    {
        ...
        if (count($args) < 2) {
            ...
            $echo[1] = static::merge(...$args);
        } else {
            $echo = $args;
        }
        return static::cover($echo);
    }
     ...
     * @return array
     */
    public static function cover($args)
    {
        // 初始化变量
        if (!is_array($args[0])) {
            $args[0] = $args[1];
            ...
            foreach ($args[1] as $key => $value) {
                if (!isset($args[0][$key])) {
                    ...
                    $args[0][$key] = static::cover([$args[0][$key], $args[1][$key]]);
                } else {
                    $args[0][$key] = $value;
                }
            }
     ...
     * @return mixed
     */
    public static function json($var, $type, $param = null)
    {
        // 初始化变量
        $echo = null;
