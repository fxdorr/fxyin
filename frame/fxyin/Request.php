...
class Request
{
    ...
    public function method($method = false)
    {
        if (true === $method) {
            // 获取原始请求类型
            return PHP_SAPI == 'cli' ? 'GET' : (isset($this->server['REQUEST_METHOD']) ? $this->server['REQUEST_METHOD'] : $_SERVER['REQUEST_METHOD']);
        } elseif (!$this->method) {
            if (isset($_POST[Config::get('var_method')])) {
                ...
                $this->method = PHP_SAPI == 'cli' ? 'GET' : (isset($this->server['REQUEST_METHOD']) ? $this->server['REQUEST_METHOD'] : $_SERVER['REQUEST_METHOD']);
            }
        }
        return $this->method;
    }
    ...
    public function cache($key, $expire = null, $except = [])
    {
        if (false !== $key && $this->isGet() && !$this->isCheckCache) {
            ...
            if (strtotime($this->server('HTTP_IF_MODIFIED_SINCE')) + $expire > $_SERVER['REQUEST_TIME']) {
                ...
                throw new \fxyin\exception\HttpResponseException($response);
